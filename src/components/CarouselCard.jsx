    ...
    <div
      className={`mx-3 ${
        clr === "1"
          ...
          ? " bg-yellow-300"
          : "bg-green-300"
      } h-[450px] text-black flex relative justify-between xs:justify-normal overflow-hidden rounded-[50px] xs:rounded-[25px] xs:flex-col xs:flex-[650px]`}
    >
      <div className="flex p-20 xs:p-5 justify-start items-start w-[60%] xs:w-full flex-col xs:h-[40%]">
        <h2 className="font-normal text-4xl md:text-6xl mb-2 md:mb-3 xs:text-2xl xs:mb-1">
          {subtitle}
        </h2>
        <h1 className="font-bold text-4xl md:text-6xl mb-2 md:mb-3 xs:text-3xl xs:mb-1">
          {title}
        </h1>
      ...
      <img
        src={pic}
        alt=""
        className="mt-4 md:mt-0 md:ml-4 xs:mt-0 xs:scale-75 xs:absolute xs:right-[-20%] xs:bottom-[-20%]"
      />
    </div>
  );
};
